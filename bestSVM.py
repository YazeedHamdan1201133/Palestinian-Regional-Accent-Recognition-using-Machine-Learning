...
def extract_features(file_path):
    try:
        ...
        mel_spect = librosa.feature.melspectrogram(y=audio, sr=sample_rate, n_mels=40)
        mel_spect_scaled = np.mean(mel_spect.T, axis=0)

        # Compute Audio Energy
        energy = np.sum(audio ** 2) / len(audio)

        # Combine MFCCs, Mel-spectrogram, and Energy into a single feature vector
        features = np.concatenate((mfccs_scaled, mel_spect_scaled, [energy]))
...
if X_train.size == 0 or X_test.size == 0:
    ...
    plt.title('Confusion Matrix "SVM2"')
    plt.show()
